              ...
              <div className="relative" ref={menuRef}>
                <button onClick={() => setShowMenu(!showMenu)} className={`flex items-center gap-3 pl-2 pr-2 lg:pl-3 lg:py-1.5 rounded-full border transition-all ${user.isAnonymous ? 'bg-slate-50 border-slate-200 hover:border-blue-300 dark:bg-slate-800 dark:border-slate-700 dark:hover:border-blue-500' : 'bg-blue-50 border-blue-200 hover:border-blue-300 dark:bg-blue-900/20 dark:border-blue-800 dark:hover:border-blue-600'}`}>
                  <div className="text-right hidden md:block">
                    <div className="text-[10px] font-black text-slate-700 dark:text-slate-200 leading-tight">{user.isAnonymous ? "GUEST USER" : user.displayName || "PRO MEMBER"}</div>
                    <div className="text-[9px] font-bold text-slate-400 dark:text-slate-500 leading-tight">{user.isAnonymous ? "Demo Mode" : "Google Account"}</div>
                  </div>
                  ...
                  <div className="absolute right-0 top-12 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-50">
                    ...
                    <div className="p-2 border-b border-slate-100 dark:border-slate-700">
                        <Link to="/profile" onClick={() => setShowMenu(false)} className="flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors group">
                            <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center group-hover:bg-blue-100 dark:group-hover:bg-blue-900 group-hover:text-blue-600"><Icons.User className="w-4 h-4" /></div>
                    ...
                    <div className="p-2 flex flex-col gap-1">
                      {user.isAnonymous ? (
                        <button onClick={handleGoogleLogin} className="flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 text-slate-700 dark:text-slate-300 hover:text-blue-700 transition-colors group">
                           <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center group-hover:border-blue-200"><Icons.User className="w-4 h-4" /></div>
                        ...
                        <div className="px-3 py-2 text-center"><span className="text-xs text-green-600 dark:text-green-400 font-bold bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded-full flex items-center justify-center gap-1"><Icons.Check className="w-3 h-3"/> Account Connected</span></div>
                      )}
                    </div>
                    <div className="p-2 border-t border-slate-100 dark:border-slate-700">
                      <button onClick={() => { onLogout(); setShowMenu(false); }} className="w-full flex items-center justify-center gap-2 text-[10px] font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 py-2 rounded-lg transition-colors"><Icons.LogOut className="w-3.5 h-3.5" />{user.isAnonymous ? "EXIT GUEST MODE" : "LOG OUT"}</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        {showMobileMenu && (
            <div ref={mobileMenuRef} className="lg:hidden absolute top-16 left-0 w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-xl z-40 animate-in slide-in-from-top-2">
                <div className="p-4 flex flex-col gap-2">
                    {navLinks.map(link => <Link key={link.to} to={link.to} onClick={() => setShowMobileMenu(false)} className={`px-4 py-3 rounded-xl text-xs font-bold transition-all flex items-center gap-3 ${isActive(link.to)}`}><link.icon className="w-4 h-4" /> {link.label}</Link>)}
                </div>
            </div>
        )}
      </nav>
