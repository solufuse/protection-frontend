...
export default function ResultsTable({ results, filterSearch, setFilterSearch, filterWinner, setFilterWinner, filterValid, setFilterValid, onSelectCase }: Props) {
    ...
    const filteredData = results.filter(r => { 
        ...
        const matchesSearch = filterSearch === "" || r.filename.toLowerCase().includes(searchLower) || r.study_case?.id.toLowerCase().includes(searchLower) || r.study_case?.config.toLowerCase().includes(searchLower) || r.study_case?.revision.toLowerCase().includes(searchLower); 
            ...
            <div className="p-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex justify-between items-center gap-4 shrink-0">
                <div className="flex items-center gap-2 flex-1">
                     <div className="relative flex-1 max-w-xs">
                        ...
                        <input type="text" placeholder="Filter scenarios..." value={filterSearch} onChange={(e) => setFilterSearch(e.target.value)} className="w-full pl-8 pr-2 py-1.5 text-[10px] border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 rounded focus:outline-none focus:border-blue-400 text-slate-600 dark:text-slate-300" />
                     </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left font-bold min-w-[900px]">
                        <thead className="bg-slate-50 dark:bg-slate-900 text-[9px] text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-700 sticky top-[33px]">
                            <tr>
