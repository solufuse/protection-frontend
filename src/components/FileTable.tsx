      ...
      <table className="w-full text-left font-bold border-collapse">
        ...
        <thead className="text-[9px] text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-50/50 dark:bg-slate-900/50 sticky top-0 z-10 backdrop-blur-sm">
          <tr>
            ...
            <th className="py-2 px-3 border-b border-slate-100 dark:border-slate-800 font-bold cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 group" onClick={() => onSort('filename')}>
            ...
            <th className="py-2 px-3 border-b border-slate-100 dark:border-slate-800 font-bold w-32 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400" onClick={() => onSort('uploaded_at')}>
            ...
            <th className="py-2 px-3 border-b border-slate-100 dark:border-slate-800 w-24 font-bold text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400" onClick={() => onSort('size')}>
        ...
        <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
          {files.length === 0 ? (
            ...
            files.map((file, i) => {
               ...
               return (
                <Fragment key={i}>
                    <tr className={`group transition-colors ${isExpanded ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                      <td className="px-3 py-1 text-center">{isConvertible && (<button onClick={() => onTogglePreview(file.filename)} className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400">{isExpanded ? <Icons.ChevronDown className="w-3.5 h-3.5"/> : <Icons.ChevronRight className="w-3.5 h-3.5"/>}</button>)}</td>
                      <td className="px-3 py-1"><div className="flex items-center gap-2"><Icons.FileText className={`w-3.5 h-3.5 ${isConvertible ? 'text-blue-500' : 'text-slate-400 dark:text-slate-600'}`} /><span className="truncate max-w-[280px] text-slate-700 dark:text-slate-300 text-[10px]" title={file.filename}>{file.filename}</span></div></td>
                      <td className="px-3 py-1 text-slate-400 dark:text-slate-500 font-mono text-[9px]"><div className="flex items-center gap-1"><Icons.Calendar className="w-3 h-3 text-slate-300 dark:text-slate-600"/> {file.uploaded_at || "-"}</div></td>
                      <td className="px-3 py-1 text-slate-400 dark:text-slate-500 font-mono text-[9px] text-center">{formatBytes(file.size)}</td>
                      <td className="px-3 py-1 text-right">
                          <div className="flex justify-end gap-1.5 items-center opacity-80 group-hover:opacity-100 transition-opacity">
                              ...
                              {isConvertible && (
                              <>
                                <button onClick={() => onOpenLink('xlsx', file.filename)} className="flex items-center gap-1 px-1.5 py-0.5 bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/40 text-green-700 dark:text-green-400 rounded border border-green-200 dark:border-green-900 transition-colors" title="Download XLSX"><Icons.FileSpreadsheet className="w-3 h-3"/> <span className="text-[9px]">XLSX</span></button>
                                <button onClick={() => onOpenLink('json', file.filename)} className="flex items-center gap-1 px-1.5 py-0.5 bg-yellow-50 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:hover:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400 rounded border border-yellow-200 dark:border-yellow-900 transition-colors" title="Download JSON"><Icons.FileJson className="w-3 h-3"/> <span className="text-[9px]">JSON</span></button>
                                <button onClick={() => onOpenLink('json_tab', file.filename)} className="flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 text-blue-700 dark:text-blue-400 rounded border border-blue-200 dark:border-blue-900 transition-colors" title="Open in new Tab"><Icons.ExternalLink className="w-3 h-3"/> <span className="text-[9px]">OPEN</span></button>
