...
export default function Sidebar({ 
  ...
  const sidebarPadding = isSidebarExpanded ? 'px-4' : 'px-2'; // Adjust padding for collapsed state
  const linkPadding = isSidebarExpanded ? 'px-3' : 'px-2.5';
  ...
  return (
    <aside className={`flex flex-col bg-white dark:bg-[#1e1e1e] border-r border-slate-200 dark:border-[#333] transition-all duration-300 ${isSidebarExpanded ? 'w-60' : 'w-16'}`}>
      ...
      <nav className={`flex-1 ${sidebarPadding} py-4 space-y-2`}>
        <button 
          onClick={onToggleFileManager} 
          className={`w-full flex items-center gap-3 ${linkPadding} py-2.5 rounded-lg text-sm font-bold transition-all ${isFileManagerOpen ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'} ${!isSidebarExpanded && 'justify-center'}`}>
          <Files className="w-5 h-5" />
          {isSidebarExpanded && <span>Files</span>}
        </button>

        {navLinks.map(link => (
          <Link key={link.to} to={link.to} title={link.label} className={`w-full flex items-center gap-3 ${linkPadding} py-2.5 rounded-lg text-sm font-bold transition-all ${isActive(link.to) ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'} ${!isSidebarExpanded && 'justify-center'}`}>
      ...
      <div className={`relative ${sidebarPadding} py-4 border-t border-slate-200 dark:border-[#333]`}>
        <div className="relative">
          <button onClick={() => setShowUserMenu(!showUserMenu)} className={`w-full flex items-center gap-3 ${!isSidebarExpanded && 'justify-center'}`}>
        ...
        <button onClick={() => setIsSidebarExpanded(!isSidebarExpanded)} className="absolute -top-8 right-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800">
