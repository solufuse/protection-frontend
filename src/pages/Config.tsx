...
import { Save, Trash2, Settings, Zap, Download, Activity, ChevronDown, ChevronRight } from 'lucide-react';
        ...
        <div className="col-span-12 lg:col-span-8 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden">
            <div className="flex justify-between items-center p-2 bg-slate-50 cursor-pointer" onClick={() => toggleSection('inrush')}>
              ...
              {openSections.inrush && <button onClick={e => { e.stopPropagation(); setConfig({...config, transformers: [...config.transformers, {name: "TX-NEW", sn_kva: 0, u_kv: 0, ratio_iencl: 0, tau_ms: 0}]})}} className="text-[9px] font-bold bg-white text-orange-600 px-2 py-0.5 rounded border border-orange-200 hover:bg-orange-50 font-bold">+ ADD TX</button>}
            </div>
            {openSections.inrush && (
              <div className="p-2 border-t border-slate-100 max-h-40 overflow-y-auto font-bold">
                  ...
                  <tbody className="divide-y divide-slate-100 font-bold">
                    {config.transformers.map((tx: any, i: number) => (
                      <tr key={i} className="hover:bg-slate-50">
                        <td><input value={tx.name} onChange={e => { const n = [...config.transformers]; n[i].name = e.target.value; setConfig({...config, transformers: n}); }} className="w-full bg-transparent text-blue-600 outline-none font-bold"/></td>
                        <td><input type="number" value={tx.sn_kva} onChange={e => { const n = [...config.transformers]; n[i].sn_kva = parseFloat(e.target.value); setConfig({...config, transformers: n}); }} className="w-full text-center bg-transparent outline-none font-bold"/></td>
                        <td><input type="number" value={tx.u_kv} onChange={e => { const n = [...config.transformers]; n[i].u_kv = parseFloat(e.target.value); setConfig({...config, transformers: n}); }} className="w-full text-center bg-transparent outline-none font-bold"/></td>
                        <td><input type="number" value={tx.ratio_iencl} onChange={e => { const n = [...config.transformers]; n[i].ratio_iencl = parseFloat(e.target.value); setConfig({...config, transformers: n}); }} className="w-full text-center bg-transparent outline-none font-bold"/></td>
                        <td><input type="number" value={tx.tau_ms} onChange={e => { const n = [...config.transformers]; n[i].tau_ms = parseFloat(e.target.value); setConfig({...config, transformers: n}); }} className="w-full text-center bg-transparent outline-none font-bold"/></td>
          ...
          <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden font-bold">
            ...
            {openSections.loadflow && (
              <div className="p-3 border-t border-slate-100 grid grid-cols-3 gap-4 font-bold">
          ...
          <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden font-bold">
            <div className="flex justify-between items-center p-2 bg-slate-50 cursor-pointer" onClick={() => toggleSection('protection')}>
              <h2 className="font-bold flex items-center gap-1.5 text-slate-700 uppercase">{openSections.protection ? <ChevronDown className="w-3 h-3"/> : <ChevronRight className="w-3 h-3"/>} <Zap className="w-3.5 h-3.5 text-blue-600" /> Protection 51</h2>
            </div>
            {openSections.protection && (
              <div className="p-3 border-t border-slate-100 space-y-4 font-bold">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {config.plans.map((p: any, i: number) => (
                      <tr key={i} className="hover:bg-slate-50 group font-bold">
                        ...
                        <td className="px-1"><input value={p.title} onChange={e => { const n = [...config.plans]; n[i].title = e.target.value; setConfig({...config, plans: n}); }} className="w-full bg-transparent outline-none border-b border-transparent focus:border-indigo-300 font-bold"/></td>
                        <td className="px-1">
                          <select value={p.type} onChange={e => { const n = [...config.plans]; n[i].type = e.target.value; setConfig({...config, plans: n}); }} className="bg-transparent outline-none text-[9px] text-slate-500 border-none font-bold">
                        ...
                        <td className="px-1"><input value={p.related_source} onChange={e => { const n = [...config.plans]; n[i].related_source = e.target.value; setConfig({...config, plans: n}); }} className="w-full bg-transparent outline-none border-b border-transparent focus:border-indigo-300 font-bold"/></td>
                        <td className="px-1">
